...
+ (void)addLastString:(NSString *)str parameter:(NSDictionary *)parame AndBack:(void(^)(NSDictionary * backDic))success failure:(void(^)(NSError * error))meg{
...
//查询充电站
+ (void)requestsearchCanBespeakChargePoleWithlat:(NSString *)lat lon:(NSString *)lon radius:(NSString *)radius type:(NSString *)type status:(NSString *)status startTime:(NSString *)startTime endTime:(NSString *)endTime regId:(NSString *)regId AndBack:(void (^)(NSDictionary * backDic))success failure:(void (^)(NSError * _Nonnull))failure{
